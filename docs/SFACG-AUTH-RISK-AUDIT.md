...
- `AuthStatus` 分别返回 `appAuthenticated` 与 `webAuthenticated`。账户资料、书架及 App 正文使用 App 会话；有声、漫画和网页正文回退使用 Web 会话。
...
- Web 登录不会被误认为 App 登录；有声功能会明确要求 Web 会话，App 资料功能会明确要求 App 会话。密码登录成功后仅为设备上报诊断读取一次 `accountId`，不会把账户资料返回给渲染层。
