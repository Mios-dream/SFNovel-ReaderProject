...
应用采用 Tauri 构建：Vue 负责界面，Rust 负责网络请求、下载任务、书库、导出和本地会话管理。正式运行时不依赖 Express、本地 API 端口、Vite 代理或 Node.js 服务进程。

> [!WARNING]
> 使用第三方客户端可能导致账号出现风控提醒，需要手动在手机端验证。使用本项目则表明理解使用风险，使用造成的后果自行承担。
