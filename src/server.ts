...
type Job = { id: string; title: string; status: "queued" | "downloading" | "paused" | "done" | "error" | "cancelled"; progress: number; message: string; file?: string; kind: "text" | "audio"; novelId: number; chapterIds?: number[] };
...
type AudioChapter = { id: number; title: string; source: string; volume: string };
...
type AudioInfoResponse = {
  ...
  data?: {
    NovelName?: string;
    VolumeSet?: Array<{ VolumeName?: string; AudioSet?: Array<{ AudioID?: number; ChapterTitle?: string; AudioSrc?: string }> }>;
  };
};
...
const safeAudioName = (name: string) => safeName(name).replace(/[. ]+$/g, "") || "未命名章节";
...
async function readNovelDownloadMetadata(dir: string): Promise<NovelDownloadMetadata> {
  try {
    const metadata = await fse.readJson(path.join(dir, ".novel-flow.json")) as NovelDownloadMetadata;
...
async function writeNovelDownloadMetadata(dir: string, metadata: NovelDownloadMetadata) {
  await fse.outputFile(path.join(dir, ".novel-flow.json"), JSON.stringify({
    ...metadata,
...
async function getLocalDownloadState(novelId: number) {
  ...
  const states = await Promise.all(folders.filter((folder) => folder.isDirectory()).map(async (folder) => {
    const dir = path.join(libraryDir, folder.name);
    const metadata = await readNovelDownloadMetadata(dir);
    if (metadata.novelId !== novelId || numberIds(metadata.downloadedTextChapterIds).length) return { metadata, downloadedTextTitles: [] as string[] };
...
function getAuthSession(req: express.Request): AuthSession | undefined {
  const token = req.headers.cookie?.split(/;\s*/).find((item) => item.startsWith(`${localAuthCookie}=`))?.slice(localAuthCookie.length + 1);
  if (!token) return undefined;
  try {
    const parsed = JSON.parse(Buffer.from(token, "base64url").toString("utf8")) as Partial<AuthSession>;
    return typeof parsed.cookie === "string" && typeof parsed.userName === "string" ? { cookie: parsed.cookie, userName: parsed.userName } : undefined;
  } catch {
    return undefined;
  }
}
...
function saveAuthSession(res: express.Response, session: AuthSession) {
  res.cookie(localAuthCookie, Buffer.from(JSON.stringify(session)).toString("base64url"), {
    httpOnly: true,
    sameSite: "strict",
...
function hasCompletedOfficialLogin(url: string) {
  try {
    ...
    return pageUrl.pathname.toLowerCase().endsWith("/message.aspx")
...
async function getControlledBrowserSession(): Promise<ControlledBrowserSession | undefined> {
  try {
    const { data } = await axios.get<DevToolsTab[]>(`http://127.0.0.1:${browserDebugPort}/json/list`, { timeout: 700 });
    ...
    const cookie = [...new Map(sfCookies
      .sort((left, right) => Number(right.domain === "api.sfacg.com") - Number(left.domain === "api.sfacg.com"))
      .map((cookie) => [cookie.name, cookie.value])).entries()]
...
async function isControlledBrowserRunning() {
  try {
    await axios.get(`http://127.0.0.1:${browserDebugPort}/json/version`, { timeout: 500 });
    return true;
  } catch {
    return false;
  }
...
async function closeControlledBrowser() {
  try {
    const { data } = await axios.get<DevToolsVersion>(`http://127.0.0.1:${browserDebugPort}/json/version`, { timeout: 700 });
...
async function writeNovel(novelId: number, cookie: string | undefined, chapterIds: number[] | undefined, signal: AbortSignal, onProgress?: (value: number, message: string) => void) {
    ...
    for (const chapter of volume.chapterList) {
      ...
      try {
        ...
        if (raw) {
          ...
          savedChapters[String(chapter.chapId)] = { volume: volume.title, title: chapter.ntitle, content };
  ...
  await writeNovelDownloadMetadata(novelDir, { ...metadata, novelId, title: novel.novelName, downloadedTextChapterIds: [...downloadedTextChapterIds] });
  if (novel.novelCover) {
    try { await fse.outputFile(path.join(imageDir, "cover.jpeg"), await SfacgClient.image(novel.novelCover)); } catch { /* cover is optional */ }
  }
...
async function getAudioChapters(novelId: number, cookie: string) {
  ...
  try {
    ...
    if (axios.isAxiosError(error) && (error.response?.status === 401 || error.response?.status === 403)) {
      throw new AudioCatalogError("AUTH_EXPIRED", "SF 登录会话已失效，请重新登录", 401);
    }
    const detail = axios.isAxiosError(error) && error.code === "ECONNABORTED" ? "SF 有声接口请求超时" : "无法连接 SF 有声接口";
  ...
  if (upstreamStatus !== 200 || !data.data) {
    ...
    throw new AudioCatalogError("UPSTREAM_ERROR", `SF 有声接口拒绝了请求${detail}`, 502);
  }
...
async function writeAudio(novelId: number, cookie: string, chapterIds: number[] | undefined, signal: AbortSignal, onProgress?: (value: number, message: string) => void) {
  ...
  const metadata = await readNovelDownloadMetadata(path.join(libraryDir, novelName));
  ...
  for (const [index, chapter] of chapters.entries()) {
    ...
    if (!await fse.pathExists(target)) {
      try {
        const response = await axios.get<NodeJS.ReadableStream>(chapter.source, {
          responseType: "stream",
          signal,
    ...
    await writeNovelDownloadMetadata(path.join(libraryDir, novelName), { ...metadata, novelId, title: audio.title, downloadedAudioChapterIds: [...downloadedAudioChapterIds] });
...
app.get("/api/search", async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ message: error instanceof Error ? error.message : "搜索失败" });
  }
});
...
app.get("/api/bookshelf", async (req, res) => {
  ...
  try {
    ...
    const books = await client.bookshelfInfos();
    if (!books) return res.status(401).json({ message: "SF 登录会话已失效，请重新登录后同步书架" });
    let typeNames = new Map<number, string>();
    try {
      const types = await client.typeInfo();
      typeNames = new Map((types || []).map((type) => [type.typeId, type.typeName]));
    } catch {
      // Older or limited sessions may not expose the official type endpoint.
    }
    res.json(books.map((book) => ({ ...book, categoryName: book.typeId ? typeNames.get(book.typeId) : undefined })));
  } catch (error) {
    res.status(502).json({ message: error instanceof Error ? `SF 书架请求失败：${error.message}` : "读取书架失败" });
  }
});
...
app.get("/api/chapters/:novelId", async (req, res) => {
  ...
  try {
    ...
    const [volumes, localState] = await Promise.all([client.volumeInfos(novelId), getLocalDownloadState(novelId)]);
    ...
    res.json(volumes.map((volume) => ({ volumeId: volume.volumeId, title: volume.title, chapters: volume.chapterList.map((chapter) => {
      const isDownloaded = downloaded.has(chapter.chapId) || downloadedTitles.has(chapter.ntitle);
    ...
    res.status(500).json({ message: error instanceof Error ? error.message : "读取章节目录失败" });
  }
});
...
app.get("/api/novel/:novelId", async (req, res) => {
  ...
  if (!Number.isInteger(novelId) || novelId <= 0) return res.status(400).json({ message: "小说编号无效" });
  try {
    const novel = await new SfacgClient().novelInfo(novelId);
    ...
    res.status(500).json({ message: error instanceof Error ? error.message : "读取小说信息失败" });
  }
});
...
app.get("/api/audio/:novelId", async (req, res) => {
  ...
  if (!Number.isInteger(novelId) || novelId <= 0) return res.status(400).json({ message: "小说编号无效" });
  try {
    const [audio, localState] = await Promise.all([getAudioChapters(novelId, session.cookie), getLocalDownloadState(novelId)]);
...
app.post("/api/auth/browser-login", async (_req, res) => {
  if (!await isControlledBrowserRunning()) {
    ...
    if (!executable) return res.status(500).json({ message: "未找到 Microsoft Edge 或 Google Chrome" });
...
app.get("/api/auth/browser-login/status", async (_req, res) => {
  ...
  if (!session) return res.json({ authenticated: false, waiting: await isControlledBrowserRunning() || Date.now() < browserLaunchPendingUntil });
  if (!session.loginCompleted) return res.json({ authenticated: false, waiting: await isControlledBrowserRunning() || Date.now() < browserLaunchPendingUntil });
...
app.post("/api/auth/logout", (_req, res) => {
  res.clearCookie(localAuthCookie, { httpOnly: true, sameSite: "strict", path: "/" });
  res.status(204).end();
});
...
function runJob(job: Job) {
  ...
  void (async () => {
    try {
      ...
      if (job.kind === "audio") {
        const saved = await writeAudio(job.novelId, session.cookie || "", job.chapterIds, controller.signal, (progress, message) => Object.assign(job, { progress, message }));
        Object.assign(job, { status: "done", progress: 100, message: `已保存 ${saved.chapters} 集有声内容`, file: bookUrl(saved.folder, saved.file) });
      } else {
        const saved = await writeNovel(job.novelId, session.cookie, job.chapterIds, controller.signal, (progress, message) => Object.assign(job, { progress, message }));
...
app.post("/api/download", (req, res) => {
  const { novelId, title, chapterIds } = req.body as { novelId?: number; title?: string; chapterIds?: number[] };
  if (!novelId) return res.status(400).json({ message: "缺少小说编号" });
  if (!validateChapterIds(chapterIds)) return res.status(400).json({ message: "章节选择无效" });
  const id = `${Date.now()}-${novelId}`;
  const job: Job = { id, title: title || `小说 ${novelId}`, kind: "text", novelId, chapterIds, status: "queued", progress: 0, message: "等待开始" };
  jobs.set(id, job); jobSessions.set(id, { cookie: getAuthSession(req)?.cookie, chapterIds }); runJob(job);
...
app.post("/api/audio/download", (req, res) => {
  ...
  const job: Job = { id, title: title || `小说 ${novelId}`, kind: "audio", novelId, chapterIds, status: "queued", progress: 0, message: "等待开始" };
  jobs.set(id, job); jobSessions.set(id, { cookie: session.cookie, chapterIds }); runJob(job);
  res.status(202).json(job);
});
...
app.post("/api/jobs/:id/pause", (req, res) => {
  ...
  if (job.status === "queued" || job.status === "downloading") { job.status = "paused"; job.message = "正在暂停下载"; jobControllers.get(job.id)?.abort(); }
  res.json(job);
});
...
app.post("/api/jobs/:id/resume", (req, res) => {
  const job = jobs.get(req.params.id);
  if (!job) return res.status(404).json({ message: "下载任务不存在" });
  if (job.status === "paused") { job.message = "等待继续"; runJob(job); }
  res.json(job);
});
...
app.post("/api/jobs/:id/cancel", (req, res) => {
  const job = jobs.get(req.params.id);
  if (!job) return res.status(404).json({ message: "下载任务不存在" });
  if (job.status === "done" || job.status === "error" || job.status === "cancelled") return res.json(job);
...
app.delete("/api/jobs/:id", (req, res) => {
  const job = jobs.get(req.params.id);
  if (!job) return res.status(404).json({ message: "下载任务不存在" });
  jobControllers.get(job.id)?.abort(); jobs.delete(job.id); jobControllers.delete(job.id); jobSessions.delete(job.id);
  res.status(204).end();
});
...
app.get("/api/library", async (_req, res) => {
  ...
  const books = await Promise.all(folders.filter((folder) => folder.isDirectory()).map(async (folder) => {
    ...
    try { metadata = await fse.readJson(path.join(dir, ".novel-flow.json")); } catch { /* metadata is optional for older downloads */ }
    return {
      name: folder.name,
      novelId: Number.isInteger(metadata.novelId) ? metadata.novelId : undefined,
...
app.listen(8787, "127.0.0.1", () => console.log("Novel Flow API: http://127.0.0.1:8787"));
