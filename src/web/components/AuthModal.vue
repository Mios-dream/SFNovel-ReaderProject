...
<template>
  <div v-if="open" class="modal-backdrop" @click.self="emit('close')"><section class="modal glass"><button class="close-button" title="关闭" @click="emit('close')"><X :size="20" /></button><span class="modal-icon"><KeyRound :size="22" /></span><h2>SF 下载会话</h2><template v-if="auth.authenticated"><p>当前账号已通过官方登录验证。会话保存在此浏览器的本地安全 Cookie 中，刷新页面或重启本地服务后仍可恢复。</p><button class="text-button" @click="emit('logout')">退出当前会话 <ChevronRight :size="15" /></button></template><template v-else><p>将打开一个独立的官方登录窗口。账号、密码和滑块验证都只在该官方页面内完成，应用会自动取得本地下载会话。</p><button class="primary-button full" :disabled="busy" @click="emit('login')"><LoaderCircle v-if="busy" class="spin" :size="18" />{{ busy ? '正在等待官方登录' : '打开官方登录窗口' }}</button></template></section></div>
</template>
